...
BEGIN
  ...
  SELECT json_build_object(
    ...
    -- FIX: Count only active teachers to match Teachers Screen
    ...
    'group_count', (SELECT COUNT(*)::int FROM groups WHERE center_id = p_center_id AND status = 'active'),
    
    'active_students', (
      SELECT COUNT(*)::int FROM student_enrollments 
      WHERE center_id = p_center_id AND status = 'accepted'
    ),
    ...
    'active_groups_count', (SELECT COUNT(*)::int FROM groups WHERE center_id = p_center_id AND status = 'active'),
    'full_groups', (SELECT COUNT(*)::int FROM groups WHERE center_id = p_center_id AND status = 'active' AND (
        SELECT COUNT(*) FROM student_group_enrollments sge WHERE sge.group_id = groups.id AND sge.status = 'active'
    ) >= max_students),

    -- 6️⃣ الجلسة القادمة (FIXED: Consider Time)
    'next_session_time', (
    ...
    'student_distribution', (
      ...
      FROM (
        ...
        WHERE se.center_id = p_center_id AND se.status = 'accepted'
